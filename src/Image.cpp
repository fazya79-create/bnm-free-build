...
std::vector<BNM::Class> Image::GetClasses(bool includeInner) const {
    ...
    if (_data->nameToClassHashTable != (decltype(_data->nameToClassHashTable)) -0x424e4d) {
        if (api.il2cpp_image_get_class_count && api.il2cpp_image_get_class) {
            ...
            for (size_t i = 0; i < typeCount; ++i) {
                ...
                if (!includeInner && declaring)
                    continue;
                auto name = api.il2cpp_class_get_name(cls);
                if (!includeInner &&
                    (!api.il2cpp_class_get_flags(cls) || (name && strcmp(name, "<Module>") == 0)))
                    continue;
    ...
    ClassesManagement::ForEachClassInImage(
        _data, [&classes, includeInner](IL2CPP::Il2CppClass *cls) -> bool {
            if (!includeInner && Internal::api.il2cpp_class_get_declaring_type(cls))
                return false;
            classes.push_back(cls);
            return false;
        });
