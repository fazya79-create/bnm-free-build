...
void MANAGEMENT_STRUCTURES::AddClass(CustomClass *_class) {
    if (!ClassesManagement::classesManagementVector)
        ClassesManagement::classesManagementVector =
            new (BNM_malloc(sizeof(std::vector<MANAGEMENT_STRUCTURES::CustomClass *>)))
...
#define BNM_I2C_NEW(type) (IL2CPP::type *) BNM_malloc(sizeof(IL2CPP::type))
    ...
    static IL2CPP::MethodInfo *ProcessCustomMethod(MANAGEMENT_STRUCTURES::CustomMethod *method,
                                                   Class target, bool *hooked = nullptr);
    ...
    static void GetAllInterfaces(IL2CPP::Il2CppClass *parent, IL2CPP::Il2CppClass *interface,
    ...
    void ProcessCustomClasses() {
        if (classesManagementVector == nullptr)
            return;

        for (auto customClass : *classesManagementVector)
            if (customClass)
                ProcessClassRuntime(customClass);

        using VectorType = std::vector<MANAGEMENT_STRUCTURES::CustomClass *>;
        classesManagementVector->~VectorType();
    ...
    void ProcessClassRuntime(MANAGEMENT_STRUCTURES::CustomClass *customClass) {
        ...
        type.Free();
        customClass->_baseType.Free();
        for (auto &interface : customClass->_interfaces) interface.Free();
    ...
    static void ModifyClass(MANAGEMENT_STRUCTURES::CustomClass *customClass, Class target) {
        ...
        if (newMethodsCount) {
            ...
            for (size_t i = 0; i < newMethodsCount; ++i) {
                ...
                BNM_LOG_DEBUG_IF(isHooked, "ModifyClass hooked method: %s (%zu)",
                                 method->_name.data(), paramCount);
                BNM_LOG_DEBUG_IF(!isHooked, "ModifyClass added method: %s (%zu)",
                                 method->_name.data(), paramCount);
            }
        ...
        if (newFieldsCount) {
            ...
            auto newFields = (IL2CPP::FieldInfo *) BNM_malloc((oldCount + newFieldsCount) *
                                                              sizeof(IL2CPP::FieldInfo));

            if (oldCount && klass->fields)
                memcpy(newFields, klass->fields, oldCount * sizeof(IL2CPP::FieldInfo));
    ...
    static void CreateClass(MANAGEMENT_STRUCTURES::CustomClass *customClass,
        ...
        if (!image)
            image = MakeImage(classInfo._imageName ? classInfo._imageName : baseImageName);
        if (!image || !image._data) {
            BNM_LOG_ERR("CreateClass failed: no image for %s",
                        classInfo._name ? classInfo._name : "(null)");
            return;
        }

        BNM_LOG_DEBUG("CreateClass target: %s.%s in %s",
        ...
        if (!parent) {
            BNM_LOG_ERR("CreateClass failed: no parent class (System.Object unresolved) for %s",
                        classInfo._name);
            return;
        }
        ...
        auto methodsCount = customClass->_methods.size();
        auto methods = methodsCount ? (const IL2CPP::MethodInfo **) BNM_malloc(
                                          methodsCount * sizeof(IL2CPP::MethodInfo *))
                                    : nullptr;
        ...
        for (size_t i = 0; i < methodsCount; ++i) {
            ...
            BNM_LOG_DEBUG("CreateClass added method: %s (%zu)", method->_name.data(),
                          method->_parameterTypes.size());

            if (!method->_isStatic)
                for (uint16_t v = 0; v < newVtableSize; ++v) {
                    auto &vTable = newVTable[v];
                    if (!vTable.method || !vTable.method->name || !method->myInfo->name)
                        continue;
                    ...
                    if (!strcmp(vTable.method->name, method->myInfo->name) &&
                        ...
                        if (!method->_skipTypeMatch) {
                            ...
                            for (uint8_t p = 0; p < count; ++p) {
                                auto type = Internal::api.il2cpp_method_get_param(
                                    (IL2CPP::MethodInfo *) vTable.method, p);
                        ...
                        if (!hasFinalize)
                            hasFinalize = v == Internal::finalizerSlot;
                        ...
                        BNM_LOG_DEBUG("CreateClass overridden virtual: %s",
        ...
        auto klass = customClass->myClass = (IL2CPP::Il2CppClass *) BNM_malloc(
            sizeof(IL2CPP::Il2CppClass) + newVTable.size() * sizeof(IL2CPP::VirtualInvokeData));
        memset(klass, 0,
               sizeof(IL2CPP::Il2CppClass) + newVTable.size() * sizeof(IL2CPP::VirtualInvokeData));
        ...
        if (klass->field_count > 0) {
            auto fields =
                (IL2CPP::FieldInfo *) BNM_malloc(klass->field_count * sizeof(IL2CPP::FieldInfo));
        ...
        if (!interfaces.empty()) {
            ...
            klass->implementedInterfaces = (IL2CPP::Il2CppClass **) BNM_malloc(
        ...
        for (auto method : customClass->_methods)
            PRIVATE_INTERNAL::GetMethodClass(method->myInfo) = klass;
        klass->method_count = (uint16_t) methodsCount;
        klass->methods = methods;
        klass->has_finalize = hasFinalize;

        static constexpr BNM_PTR BNM_CLASS_ALLOCATED_ALL_FLAGS =
            BNM_CLASS_ALLOCATED_METHODS_FLAG | BNM_CLASS_ALLOCATED_FIELDS_FLAG |
            BNM_CLASS_ALLOCATED_INNER_FLAG | BNM_CLASS_ALLOCATED_HIERARCHY_FLAG;
        auto ownAllocatedFlags = klass->flags & (uint32_t) BNM_CLASS_ALLOCATED_ALL_FLAGS;
        klass->flags = ((klass->parent->flags & ~(0x00000080 | 0x00000020)) &
                        ~(uint32_t) BNM_CLASS_ALLOCATED_ALL_FLAGS) |
                       0x0000001 | ownAllocatedFlags;
        if (methods)
            klass->flags |= (uint32_t) BNM_CLASS_ALLOCATED_METHODS_FLAG;
        if (klass->fields)
            klass->flags |= (uint32_t) BNM_CLASS_ALLOCATED_FIELDS_FLAG;
        ...
        auto instanceSize = customClass->_size;
        if (instanceSize < sizeof(IL2CPP::Il2CppObject))
            instanceSize = sizeof(IL2CPP::Il2CppObject);
        klass->instance_size = klass->actualSize = (uint32_t) instanceSize;
        ...
        if (!newInterOffsets.empty()) {
            klass->interfaceOffsets = (IL2CPP::Il2CppRuntimeInterfaceOffsetPair *) BNM_malloc(
    ...
    static CustomClassInfo GetClassInfo(const CompileTimeClass &compileTimeClass) {
        ...
        for (size_t i = 0; i < stack.size(); ++i) {
            auto &info = stack[i];
            if (!info || info->_baseType != CompileTimeClass::_BaseType::Class)
                continue;
            ...
            for (size_t j = i + 1; j < stack.size(); ++j) {
                if (stack[j] && stack[j]->_baseType == CompileTimeClass::_BaseType::Class) {
                    isLastClass = false;
                    break;
                }
            }
            ...
            if (isLastClass)
                return {ns, name, classInfo->_imageName, Class{}};
            return {};
        }
        return {nullptr, nullptr, nullptr, compileTimeClass.ToClass()};
    }
    ...
    static IL2CPP::Il2CppImage *MakeImage(std::string_view imageName) {
        ...
        BNM_LOG_INFO("MakeImage added: %s", newImg->name);

        return newImg;
    }

    static IL2CPP::VirtualInvokeData *TryFindVirtualMethod(Class target,
                                                           IL2CPP::MethodInfo *targetMethod,
                                                           uint16_t *outSlot = nullptr) {
        if (target._data && targetMethod)
    ...
    static IL2CPP::MethodInfo *ProcessCustomMethod(MANAGEMENT_STRUCTURES::CustomMethod *method,
        ...
        auto originalMethod =
            Internal::IterateMethods(target, [method, parameters](IL2CPP::MethodInfo *klassMethod) {
                if (method->_name != (klassMethod->name ? klassMethod->name : "") ||
                ...
                for (uint8_t i = 0; i < parameters; ++i) {
                    ...
                    if (Class(param).GetClass() != method->_parameterTypes[i].ToIl2CppClass())
                        return false;
                }
                return true;
            });
        ...
        if (!originalMethod || PRIVATE_INTERNAL::GetMethodClass(originalMethod) != target._data) {
            ...
            auto newMethod = CreateMethod(method);

            if (!method->_copyTarget.empty()) {
                auto copyTarget = Internal::IterateMethods(
                    target, [targetName = method->_copyTarget](IL2CPP::MethodInfo *method) {
                        return targetName == (method->name ? method->name : "");
                    });
                if (copyTarget)
                    newMethod->token = copyTarget->token;
            }
        ...
        if (auto vTable = TryFindVirtualMethod(target, originalMethod);
            vTable != nullptr && vTable->method != nullptr) {
            method->_origin = (IL2CPP::MethodInfo *) vTable->method;
    ...
    SKIP_NON_VIRTUAL:
        ...
        if (!originalMethod->methodPointer) {
            BNM_LOG_ERR("ProcessCustomMethod: no original code to hook for %s",
                        method->_name.data());
            return originalMethod;
        }
    ...
    static void SetupField(IL2CPP::FieldInfo *newField, MANAGEMENT_STRUCTURES::CustomField *field) {
        ...
        memset(newType, 0, sizeof(IL2CPP::Il2CppType));
        if (auto il2cppType = fieldType ? fieldType.GetIl2CppType() : nullptr; il2cppType)
            *newType = *il2cppType;
    ...
    static void SetupClassOwner(IL2CPP::Il2CppClass *target, IL2CPP::Il2CppClass *owner) {
        ...
        auto newInnerList = (IL2CPP::Il2CppClass **) BNM_malloc(sizeof(IL2CPP::Il2CppClass *) *
                                                                (owner->nested_type_count + 1));
        if (owner->nested_type_count && oldInnerList)
        ...
        if ((owner->flags & BNM_CLASS_ALLOCATED_INNER_FLAG) == BNM_CLASS_ALLOCATED_INNER_FLAG)
            BNM_free(oldInnerList);
        owner->flags |= (uint32_t) BNM_CLASS_ALLOCATED_INNER_FLAG;

        if (oldOwner && oldOwner != owner && oldOwner->nested_type_count && oldOwner->nestedTypes) {
            oldInnerList = oldOwner->nestedTypes;

            uint16_t found = 0;
            for (uint16_t i = 0; i < oldOwner->nested_type_count; ++i)
                if (oldInnerList[i] == target) {
                    found = 1;
                    break;
                }
            if (!found)
            ...
            newInnerList = (IL2CPP::Il2CppClass **) BNM_malloc(sizeof(IL2CPP::Il2CppClass *) *
                                                               (oldOwner->nested_type_count - 1));
            uint16_t skipped = 0;
            for (uint16_t i = 0; i < oldOwner->nested_type_count; ++i) {
                if (!skipped && oldInnerList[i] == target) {
                    skipped = 1;
                    continue;
                }
    ...
    static void SetupClassParent(IL2CPP::Il2CppClass *target, IL2CPP::Il2CppClass *parent) {
        ...
        target->typeHierarchy = (IL2CPP::Il2CppClass **) BNM_malloc(target->typeHierarchyDepth *
                                                                    sizeof(IL2CPP::Il2CppClass *));
        if (parent->typeHierarchyDepth)
    ...
    static void GetAllInterfaces(IL2CPP::Il2CppClass *parent, IL2CPP::Il2CppClass *interface,
        ...
        for (auto existing : outInterfaces)
            if (existing == interface)
                return;
        if (!HasInterface(parent, interface))
            outInterfaces.push_back(interface);
        if (!interface->interfaces_count || interface->interfaces_count == (uint16_t) -1 ||
            !interface->implementedInterfaces)
            return;

        for (uint16_t i = 0; i < interface->interfaces_count; ++i)
            GetAllInterfaces(parent, interface->implementedInterfaces[i], outInterfaces);
    }

    static bool HasInterface(IL2CPP::Il2CppClass *parent, IL2CPP::Il2CppClass *interface) {
        if (!parent || !interface)
            return false;
        if (parent->implementedInterfaces && parent->interfaces_count != (uint16_t) -1)
    ...
    IL2CPP::Il2CppClass *(*old_Class$$FromName)(IL2CPP::Il2CppImage *image, const char *namespace_,
                                                const char *name) {};
    ...
    IL2CPP::Il2CppClass *Class$$FromName(IL2CPP::Il2CppImage *image, const char *namespace_,
        ...
        if (image->nameToClassHashTable != (decltype(image->nameToClassHashTable)) -0x424e4d)
            ret = old_Class$$FromName ? old_Class$$FromName(image, namespace_, name)
                                      : api.il2cpp_class_from_name(image, namespace_, name);

        if (!ret)
            ForEachClassInImageImpl(image,
                                    [namespace_, name, &ret](IL2CPP::Il2CppClass *cls) -> bool {
                                        auto ns = api.il2cpp_class_get_namespace(cls);
                                        auto clsName = api.il2cpp_class_get_name(cls);
                                        if (!strcmp(namespace_ ? namespace_ : "", ns ? ns : "") &&
                                            !strcmp(name, clsName ? clsName : "")) {
                                            ret = cls;
                                            return true;
                                        }
                                        return false;
...
namespace BNM::Internal {
    ...
    void (*orig_Image$$GetTypes)(const IL2CPP::Il2CppImage *image, bool exportedOnly,
                                 std::vector<BNM::IL2CPP::Il2CppClass *> *target){};

    void Image$$GetTypes(const IL2CPP::Il2CppImage *image, bool,
                         std::vector<BNM::IL2CPP::Il2CppClass *> *target) {
        if (!image || !target)
            return;
        if (image->nameToClassHashTable != (decltype(image->nameToClassHashTable)) -0x424e4d &&
        ...
        if (it == ClassesManagement::bnmClassesMap.end())
            return;
