...
void ProcessCustomClasses() {
    ...
    for (auto customClass : *classesManagementVector) if (customClass) ProcessClassRuntime(customClass);
...
static void ModifyClass(MANAGEMENT_STRUCTURES::CustomClass *customClass, Class target) {
    ...
    if (newFieldsCount) {
        ...
        if (oldCount && klass->fields) memcpy(newFields, klass->fields, oldCount * sizeof(IL2CPP::FieldInfo));
...
static void CreateClass(MANAGEMENT_STRUCTURES::CustomClass *customClass, const CustomClassInfo &classInfo) {
    ...
    if (!image) image = MakeImage(classInfo._imageName ? classInfo._imageName : baseImageName);
    if (!image || !image._data) {
        BNM_LOG_ERR("CreateClass failed: no image for %s", classInfo._name ? classInfo._name : "(null)");
        return;
    }
    ...
    if (!parent) {
        BNM_LOG_ERR("CreateClass failed: no parent class (System.Object unresolved) for %s", classInfo._name);
        return;
    }
    ...
    auto methods = methodsCount ? (const IL2CPP::MethodInfo **) BNM_malloc(methodsCount * sizeof(IL2CPP::MethodInfo *)) : nullptr;
    ...
    for (size_t i = 0; i < methodsCount; ++i) {
        ...
        if (!method->_isStatic) for (uint16_t v = 0; v < newVtableSize; ++v) {
            ...
            if (!vTable.method || !vTable.method->name || !method->myInfo->name) continue;
    ...
    static constexpr BNM_PTR BNM_CLASS_ALLOCATED_ALL_FLAGS = BNM_CLASS_ALLOCATED_METHODS_FLAG | BNM_CLASS_ALLOCATED_FIELDS_FLAG | BNM_CLASS_ALLOCATED_INNER_FLAG | BNM_CLASS_ALLOCATED_HIERARCHY_FLAG;
    auto ownAllocatedFlags = klass->flags & (uint32_t) BNM_CLASS_ALLOCATED_ALL_FLAGS;
    klass->flags = ((klass->parent->flags & ~(0x00000080 | 0x00000020)) & ~(uint32_t) BNM_CLASS_ALLOCATED_ALL_FLAGS) | 0x0000001 | ownAllocatedFlags;
    if (methods) klass->flags |= (uint32_t) BNM_CLASS_ALLOCATED_METHODS_FLAG;
    if (klass->fields) klass->flags |= (uint32_t) BNM_CLASS_ALLOCATED_FIELDS_FLAG;

    klass->native_size = -1;
    klass->element_size = 0;
    auto instanceSize = customClass->_size;
    if (instanceSize < sizeof(IL2CPP::Il2CppObject)) instanceSize = sizeof(IL2CPP::Il2CppObject);
    ...
    } else klass->interfaceOffsets = nullptr;
...
static CustomClassInfo GetClassInfo(const CompileTimeClass &compileTimeClass) {
    ...
    for (size_t i = 0; i < stack.size(); ++i) {
        auto &info = stack[i];
        if (!info || info->_baseType != CompileTimeClass::_BaseType::Class) continue;

        auto classInfo = (CompileTimeClass::_ClassInfo *) info.get();
        if (!classInfo->_name) continue;
        ...
        if (isLastClass) return {ns, name, classInfo->_imageName, Class{}};
        return {};
    }
...
static IL2CPP::Il2CppImage *MakeImage(std::string_view imageName) {
    ...
    if (nameLen) memcpy((void *) newImg->nameNoExt, (const void *) imageName.data(), nameLen);
    ((char *) newImg->nameNoExt)[nameLen] = 0;

    newImg->name = (char *) BNM_malloc(nameLen + 5);
    if (nameLen) memcpy((void *) newImg->name, (const void *) imageName.data(), nameLen);
...
static IL2CPP::VirtualInvokeData *TryFindVirtualMethod(Class target, IL2CPP::MethodInfo *targetMethod, uint16_t *outSlot = nullptr) {
    if (target._data && targetMethod) for (uint16_t i = 0; i < target._data->vtable_count; ++i) {
...
static IL2CPP::MethodInfo *ProcessCustomMethod(MANAGEMENT_STRUCTURES::CustomMethod *method, Class target, bool *hooked) {
    ...
    if (!originalMethod || PRIVATE_INTERNAL::GetMethodClass(originalMethod) != target._data) {
        ...
        if (!method->_copyTarget.empty()) {
            ...
            if (copyTarget) newMethod->token = copyTarget->token;
        }
    ...
    if (auto vTable = TryFindVirtualMethod(target, originalMethod); vTable != nullptr && vTable->method != nullptr) {
        method->_origin = (IL2CPP::MethodInfo *) vTable->method;
    ...
    if (!originalMethod->methodPointer) {
        BNM_LOG_ERR("ProcessCustomMethod: no original code to hook for %s", method->_name.data());
        return originalMethod;
    }
...
static IL2CPP::MethodInfo *CreateMethod(MANAGEMENT_STRUCTURES::CustomMethod *method) {
    ...
    if (argsCount) {
        ...
        for (uint8_t p = 0; p < argsCount; ++p) {
            ...
            if (auto il2cppType = type ? type.GetIl2CppType() : nullptr; il2cppType) *parameter = *il2cppType;
...
static void SetupField(IL2CPP::FieldInfo *newField, MANAGEMENT_STRUCTURES::CustomField *field) {
    ...
    if (len) memcpy((void *) newField->name, name.data(), len);
    ...
    if (auto il2cppType = fieldType ? fieldType.GetIl2CppType() : nullptr; il2cppType) *newType = *il2cppType;
...
static void SetupClassOwner(IL2CPP::Il2CppClass *target, IL2CPP::Il2CppClass *owner) {
    ...
    if (owner->nested_type_count && oldInnerList) memcpy(newInnerList, oldInnerList, sizeof(IL2CPP::Il2CppClass *) * owner->nested_type_count);
    ...
    if (oldOwner && oldOwner != owner && oldOwner->nested_type_count && oldOwner->nestedTypes) {
        ...
        uint16_t found = 0;
        for (uint16_t i = 0; i < oldOwner->nested_type_count; ++i) if (oldInnerList[i] == target) { found = 1; break; }
        if (!found) return;
        ...
        for (uint16_t i = 0; i < oldOwner->nested_type_count; ++i) {
            if (!skipped && oldInnerList[i] == target) { skipped = 1; continue; }
...
static void GetAllInterfaces(IL2CPP::Il2CppClass *parent, IL2CPP::Il2CppClass *interface, std::vector<IL2CPP::Il2CppClass *> &outInterfaces) {
    ...
    for (auto existing : outInterfaces) if (existing == interface) return;
    if (!HasInterface(parent, interface)) outInterfaces.push_back(interface);
    if (!interface->interfaces_count || interface->interfaces_count == (uint16_t) - 1 || !interface->implementedInterfaces) return;
...
IL2CPP::Il2CppClass *Class$$FromName(IL2CPP::Il2CppImage *image, const char *namespace_, const char *name) {
    ...
    if (!ret) ForEachClassInImageImpl(image, [namespace_, name, &ret](IL2CPP::Il2CppClass *cls) -> bool {
        ...
        if (!strcmp(namespace_ ? namespace_ : "", ns ? ns : "") && !strcmp(name, clsName ? clsName : "")) {
            ret = cls;
            return true;
        }
        return false;
