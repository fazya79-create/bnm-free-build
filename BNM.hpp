...
namespace BNM_ShadowHookInternal {
    ...
    inline void EnsureInit() {
        std::call_once(initFlag, [] { shadowhook_init(SHADOWHOOK_MODE_UNIQUE, false); });
    }
}  // namespace BNM_ShadowHookInternal
template <typename PTR_T, typename NEW_T, typename T_OLD>
inline void *BasicHook(PTR_T ptr, NEW_T newMethod, T_OLD &oldBytes) {
...
namespace BNM {
    ...
    inline bool VirtualHook(BNM::Class targetClass, const BNM::MethodBase &targetMethod,
                            T_NEW newMet, T_OLD &oldMet) {
        return VirtualHookImpl(targetClass, targetMethod.GetInfo(), (void *) newMet,
                               (void **) &oldMet);
    }
    template <typename T_NEW, typename T_OLD>
    inline bool VirtualHook(BNM::Class targetClass, const BNM::MethodBase &targetMethod,
    ...
    namespace Internal {
        ...
        IL2CPP::MethodInfo *IterateMethods(Class target, CompareMethod compare) {
            ...
            if (parent)
                return IterateMethods(Class(parent), compare);
            return nullptr;
        }
        ...
        inline bool CompareImageName(IL2CPP::Il2CppImage *image, const std::string_view &name) {
            ...
            if (image->name && name == image->name)
                return true;
            if (image->nameNoExt && name == image->nameNoExt)
                return true;
            if (image->name) {
                std::string_view full(image->name);
                if (full.size() > 4 && full.substr(full.size() - 4) == ".dll" &&
                    full.substr(0, full.size() - 4) == name)
                    return true;
            }
            return false;
        }
        ...
        inline std::vector<IL2CPP::Il2CppAssembly *> &GetAllAssemblies() {
            ...
            if (size == lastDomainAssembliesCount && !assembliesCache.empty())
                return assembliesCache;
            ...
            for (size_t i = 0; i < size; ++i)
                if (assemblies[i])
                    assembliesCache.push_back(assemblies[i]);
    ...
    inline Class::Class(const CompileTimeClass &compileTimeClass) {
        _data = compileTimeClass.ToClass()._data;
    }
...
#pragma GCC diagnostic pop
    ...
    inline IL2CPP::Il2CppObject *Class::CreateNewObjectParameters(Args &&...args) const {
        ...
        if (!method || !method->methodPointer)
            return obj;
        ((void (*)(IL2CPP::Il2CppObject *, Args...)) method->methodPointer)(
    ...
    namespace Structures::Mono {
        ...
        inline Array<T> *Array<T>::Create(IL2CPP::il2cpp_array_size_t size, bool zeroed) {
            ...
            if (zeroed && size)
                memset((char *) arr + sizeof(IL2CPP::Il2CppArray), 0, size * sizeof(T));
            return arr;
        }
