...
namespace BNM {
    struct CompileTimeClass {
        ...
        inline ~CompileTimeClass() {
            if (_autoFree)
                Free();
        }

        inline void AddClass(const char *_namespace, const char *name,
        ...
        inline void AddGeneric(const std::vector<CompileTimeClass> &types) {
            ...
            for (const auto &type : types)
                info->_types.push_back(std::make_shared<CompileTimeClass>(type));
            _stack.push_back(std::move(info));
        }
        ...
        inline BNM::MethodBase operator[](const std::string_view &name) const {
            return ToClass().GetMethod(name);
        }
        inline BNM::FieldBase operator[](const char *name) const {
            return ToClass().GetField(name);
        }
        inline void Free() {
            _stack.clear();
    ...
    struct CompileTimeClassBuilder {
        ...
        inline CompileTimeClassBuilder(const char *_namespace, const char *name) {
            _class.AddClass(_namespace, name);
        }
        inline CompileTimeClassBuilder(const char *_namespace, const char *name,
                                       const char *imageName) {
            _class.AddClass(_namespace, name, imageName);
        }
        inline CompileTimeClassBuilder &Inner(const char *name) {
            _class.AddInnerClass(name);
            return *this;
        }
        inline CompileTimeClassBuilder &Pointer() {
            _class.AddModifier(CompileTimeClass::ModifierType::Pointer);
            return *this;
        }
        inline CompileTimeClassBuilder &Reference() {
            _class.AddModifier(CompileTimeClass::ModifierType::Reference);
            return *this;
        }
        inline CompileTimeClassBuilder &Array() {
            _class.AddModifier(CompileTimeClass::ModifierType::Array);
            return *this;
        }
        inline CompileTimeClassBuilder &Generic(const std::vector<CompileTimeClass> &types) {
            _class.AddGeneric(types);
            return *this;
        }
