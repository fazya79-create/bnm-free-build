...
namespace BNM {
    ...
    struct Method : public MethodBase {
        ...
        inline Method(const IL2CPP::Il2CppReflectionMethod *reflectionMethod)
        ...
        inline Ret Call(Parameters... parameters) const {
            ...
            auto method = _data;
            if (!_isStatic)
                return ((Ret (*)(IL2CPP::Il2CppObject *, Parameters...)) method->methodPointer)(
        ...
        template <typename... Parameters>
        inline IL2CPP::Il2CppObject *CreateNewObjectTypes(
            const std::initializer_list<std::string_view> &parameterNames,
            Parameters... parameters) const {
            if (!_data || !_data->klass)
                return nullptr;
        ...
        inline Ret Invoke() const {
            if (!_data || !Internal::api.il2cpp_runtime_invoke)
                return PRIVATE_INTERNAL::ReturnEmpty<Ret>();
            IL2CPP::Il2CppException *exc = nullptr;
            auto ret = Internal::api.il2cpp_runtime_invoke(_data, _instance, nullptr, &exc);
            if (exc)
                BNM_LOG_ERR("Method::Invoke exception: %s",
                            exc->message
                                ? ((Structures::Mono::String *) exc->message)->str().c_str()
                                : "unknown");
            if constexpr (!std::is_void_v<Ret>) {
                if constexpr (std::is_pointer_v<Ret>)
                    return (Ret) ret;
                else {
                    if (!ret || !Internal::api.il2cpp_object_unbox)
                        return Ret{};
        ...
        inline Ret Invoke(Parameters... parameters) const {
            ...
            IL2CPP::Il2CppException *exc = nullptr;
            auto ret = Internal::api.il2cpp_runtime_invoke(
                _data, _instance, sizeof...(Parameters) > 0 ? (void **) boxed : nullptr, &exc);
            if (exc)
                BNM_LOG_ERR("Method::Invoke exception: %s",
                            exc->message
                                ? ((Structures::Mono::String *) exc->message)->str().c_str()
       ...
       private:
        template <typename T>
        static IL2CPP::Il2CppObject *BoxInvokeArg(IL2CPP::MethodInfo *method, size_t index,
                                                  T value) {
            if constexpr (std::is_pointer_v<T>) {
                return (IL2CPP::Il2CppObject *) value;
            } else {
                if (!Internal::api.il2cpp_method_get_param ||
                    !Internal::api.il2cpp_class_from_il2cpp_type || !Internal::api.il2cpp_value_box)
                    return nullptr;
                ...
                static_assert(sizeof(T) <= sizeof(storage),
                              "Invoke() cannot box a value this large");
