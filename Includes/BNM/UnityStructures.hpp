...
namespace BNM {
    namespace Structures {
        namespace Unity {
            ...
            Vector2 Vector2::RotateTowards(Vector2 current, Vector2 target, float maxRadiansDelta,
                                           float maxMagnitudeDelta) {
                float magCur = Magnitude(current);
                float magTar = Magnitude(target);
                float newMag =
                    magCur + maxMagnitudeDelta * (float) ((magTar > magCur) - (magCur > magTar));
                ...
                Vector2 newVector = current * cosf(maxRadiansDelta) +
            ...
            Vector2 Vector2::SmoothDamp(Vector2 current, Vector2 target, Vector2 &currentVelocity,
                ...
                if (origMinusCurrent.x * outMinusOrig.x + origMinusCurrent.y * outMinusOrig.y >
                    0.f) {
            ...
            Vector3 Vector3::RotateTowards(Vector3 current, Vector3 target, float maxRadiansDelta,
                                           float maxMagnitudeDelta) {
                float magCur = Magnitude(current);
                float magTar = Magnitude(target);
                float newMag =
                ...
                current = Normalize(current);
                Vector3 newVector =
                    current * cosf(maxRadiansDelta) + Cross(axis, current) * sinf(maxRadiansDelta);
                return newVector * newMag;
            }
            ...
            float Vector3::SignedAngle(Vector3 from, Vector3 to, Vector3 axis) {
                ...
                float sign =
                    (axis.x * cross_x + axis.y * cross_y + axis.z * cross_z) >= 0.f ? 1.f : -1.f;
            ...
            struct Vector4 {
                ...
                inline static bool IsFinite(Vector4 f) {
                    return Unity::IsFinite(f.x) && Unity::IsFinite(f.y) && Unity::IsFinite(f.z) &&
                           Unity::IsFinite(f.w);
                }
            ...
            struct Color {
                ...
                [[nodiscard]] inline std::string str() const {
                    return std::string("r: ") + std::to_string(r) + std::string(", g: ") +
                           std::to_string(g) + std::string(", b: ") + std::to_string(b) +
            ...
            Quaternion Quaternion::FromToRotation(Vector3 fromVector, Vector3 toVector) {
                ...
                float k =
                    sqrtf(Vector3::SqrMagnitude(fromVector) * Vector3::SqrMagnitude(toVector));
            ...
            struct Ray {
                ...
                inline std::string str() {
                    return std::string("m_Origin: ") + m_Origin.str() +
                           std::string(", m_Direction: ") + m_Direction.str();
                }
            };
            ...
            struct RaycastHit {
                ...
                inline std::string str() {
                    return std::string("Point: ") + point.str() + std::string(", Normal: ") +
                           normal.str() + std::string(", FaceID: ") + std::to_string(faceID) +
                           std::string(", Distance: ") + std::to_string(distance) +
                           std::string(", UV: ") + UV.str() + std::string(", m_Collider: ") +
                           std::to_string((BNM_PTR) m_Collider);
                }
            };
            ...
            struct RaycastHit2D {
                ...
                inline std::string str() {
                    return std::string("centroid: ") + centroid.str() + std::string(", point: ") +
                           point.str() + std::string(", normal: ") + normal.str() +
                           std::string(", Distance: ") + std::to_string(distance) +
                           std::string(", fraction: ") + std::to_string(fraction) +
                           std::string(", m_Collider: ") + std::to_string((BNM_PTR) m_Collider);
                }
            };
            ...
            struct Matrix3x3 {
                ...
                inline Matrix3x3(float m00, float m01, float m02, float m10, float m11, float m12,
                ...
                [[nodiscard]] inline Vector3 MultiplyPoint3Transpose(const Vector3 &inV) const {
    ...
    do {                                            \
        ...
        return false;                               \
            ...
            struct Matrix4x4 {
                ...
                [[nodiscard]] inline Vector3 MultiplyPoint(const Vector3 &point) const {
                    ...
                    result.y = m_Data[1] * point.x + m_Data[5] * point.y + m_Data[9] * point.z +
                               m_Data[13];
                    result.z = m_Data[2] * point.x + m_Data[6] * point.y + m_Data[10] * point.z +
                               m_Data[14];
                ...
                [[nodiscard]] inline Vector3 InverseMultiplyVector3Affine(
                    const Vector3 &inV) const {
                    Vector3 res;
                    res.x = Get(0, 0) * inV.x + Get(1, 0) * inV.y + Get(2, 0) * inV.z;
                    res.y = Get(0, 1) * inV.x + Get(1, 1) * inV.y + Get(2, 1) * inV.z;
                    res.z = Get(0, 2) * inV.x + Get(1, 2) * inV.y + Get(2, 2) * inV.z;
            ...
            inline Matrix4x4 operator*(const Matrix4x4 &lhs, const Matrix4x4 &rhs) {
                Matrix4x4 res;
                res.m_Data[0] = lhs.m_Data[0] * rhs.m_Data[0] + lhs.m_Data[4] * rhs.m_Data[1] +
                                lhs.m_Data[8] * rhs.m_Data[2] + lhs.m_Data[12] * rhs.m_Data[3];
                res.m_Data[4] = lhs.m_Data[0] * rhs.m_Data[4] + lhs.m_Data[4] * rhs.m_Data[5] +
                                lhs.m_Data[8] * rhs.m_Data[6] + lhs.m_Data[12] * rhs.m_Data[7];
                res.m_Data[8] = lhs.m_Data[0] * rhs.m_Data[8] + lhs.m_Data[4] * rhs.m_Data[9] +
                                lhs.m_Data[8] * rhs.m_Data[10] + lhs.m_Data[12] * rhs.m_Data[11];
                res.m_Data[12] = lhs.m_Data[0] * rhs.m_Data[12] + lhs.m_Data[4] * rhs.m_Data[13] +
                                 lhs.m_Data[8] * rhs.m_Data[14] + lhs.m_Data[12] * rhs.m_Data[15];
                res.m_Data[1] = lhs.m_Data[1] * rhs.m_Data[0] + lhs.m_Data[5] * rhs.m_Data[1] +
                                lhs.m_Data[9] * rhs.m_Data[2] + lhs.m_Data[13] * rhs.m_Data[3];
                res.m_Data[5] = lhs.m_Data[1] * rhs.m_Data[4] + lhs.m_Data[5] * rhs.m_Data[5] +
                                lhs.m_Data[9] * rhs.m_Data[6] + lhs.m_Data[13] * rhs.m_Data[7];
                res.m_Data[9] = lhs.m_Data[1] * rhs.m_Data[8] + lhs.m_Data[5] * rhs.m_Data[9] +
                                lhs.m_Data[9] * rhs.m_Data[10] + lhs.m_Data[13] * rhs.m_Data[11];
                res.m_Data[13] = lhs.m_Data[1] * rhs.m_Data[12] + lhs.m_Data[5] * rhs.m_Data[13] +
                                 lhs.m_Data[9] * rhs.m_Data[14] + lhs.m_Data[13] * rhs.m_Data[15];
                res.m_Data[2] = lhs.m_Data[2] * rhs.m_Data[0] + lhs.m_Data[6] * rhs.m_Data[1] +
                                lhs.m_Data[10] * rhs.m_Data[2] + lhs.m_Data[14] * rhs.m_Data[3];
                res.m_Data[6] = lhs.m_Data[2] * rhs.m_Data[4] + lhs.m_Data[6] * rhs.m_Data[5] +
                                lhs.m_Data[10] * rhs.m_Data[6] + lhs.m_Data[14] * rhs.m_Data[7];
                res.m_Data[10] = lhs.m_Data[2] * rhs.m_Data[8] + lhs.m_Data[6] * rhs.m_Data[9] +
                                 lhs.m_Data[10] * rhs.m_Data[10] + lhs.m_Data[14] * rhs.m_Data[11];
                res.m_Data[14] = lhs.m_Data[2] * rhs.m_Data[12] + lhs.m_Data[6] * rhs.m_Data[13] +
                                 lhs.m_Data[10] * rhs.m_Data[14] + lhs.m_Data[14] * rhs.m_Data[15];
                res.m_Data[3] = lhs.m_Data[3] * rhs.m_Data[0] + lhs.m_Data[7] * rhs.m_Data[1] +
                                lhs.m_Data[11] * rhs.m_Data[2] + lhs.m_Data[15] * rhs.m_Data[3];
                res.m_Data[7] = lhs.m_Data[3] * rhs.m_Data[4] + lhs.m_Data[7] * rhs.m_Data[5] +
                                lhs.m_Data[11] * rhs.m_Data[6] + lhs.m_Data[15] * rhs.m_Data[7];
                res.m_Data[11] = lhs.m_Data[3] * rhs.m_Data[8] + lhs.m_Data[7] * rhs.m_Data[9] +
                                 lhs.m_Data[11] * rhs.m_Data[10] + lhs.m_Data[15] * rhs.m_Data[11];
                res.m_Data[15] = lhs.m_Data[3] * rhs.m_Data[12] + lhs.m_Data[7] * rhs.m_Data[13] +
            ...
            inline void MultiplyMatrices3x4(const Matrix4x4 &lhs, const Matrix4x4 &rhs,
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    res.m_Data[i + 12] = lhs.m_Data[i] * rhs.m_Data[12] +
                                         lhs.m_Data[i + 4] * rhs.m_Data[13] +
                                         lhs.m_Data[i + 8] * rhs.m_Data[14] + lhs.m_Data[i + 12];
                }
            ...
            inline void MultiplyMatrices2D(const Matrix4x4 &lhs, const Matrix4x4 &rhs,
                                           Matrix4x4 &res) {
                res.m_Data[0] = lhs.m_Data[0] * rhs.m_Data[0] + lhs.m_Data[0 + 4] * rhs.m_Data[1] +
                                lhs.m_Data[0 + 8] * rhs.m_Data[2];
                res.m_Data[4] = lhs.m_Data[0] * rhs.m_Data[4] + lhs.m_Data[0 + 4] * rhs.m_Data[5] +
                ...
                res.m_Data[1] = lhs.m_Data[1] * rhs.m_Data[0] + lhs.m_Data[1 + 4] * rhs.m_Data[1] +
                                lhs.m_Data[1 + 8] * rhs.m_Data[2];
                res.m_Data[5] = lhs.m_Data[1] * rhs.m_Data[4] + lhs.m_Data[1 + 4] * rhs.m_Data[5] +
                ...
                res.m_Data[2] = lhs.m_Data[2] * rhs.m_Data[0] + lhs.m_Data[2 + 4] * rhs.m_Data[1] +
...
#undef RETURN_ZERO
            ...
            inline void TransformPoints3x3(const Matrix4x4 &matrix, const Vector3 *in, Vector3 *out,
                                           int count) {
                auto m = Matrix3x3(matrix);
                for (int i = 0; i < count; i++) out[i] = m.MultiplyPoint3(in[i]);
            }
            inline void TransformPoints3x4(const Matrix4x4 &matrix, const Vector3 *in, Vector3 *out,
                                           int count) {
    ...
    namespace UnityEngine {
        ...
        struct Object : public BNM::IL2CPP::Il2CppObject {
            ...
            [[nodiscard]] inline bool IsValid() const {
                return BNM::Utils::CheckForNull(this) && m_CachedPtr;
            }
