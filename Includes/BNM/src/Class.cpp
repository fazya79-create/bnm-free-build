...
IL2CPP::Il2CppClass *BNM::Internal::TryGetClassWithoutImage(const std::string_view &_namespace,
...
Class::Class(const std::string_view &_namespace, const std::string_view &_name) {
    if (_data = Internal::TryGetClassWithoutImage(_namespace, _name); _data)
        return;
    BNM_LOG_WARN("Class not found: %s.%s", std::string(_namespace).c_str(),
...
Class::Class(const std::string_view &_namespace, const std::string_view &_name,
    ...
    if (!image) {
        BNM_LOG_WARN("Class image invalid: %s for %s.%s", image.str().c_str(),
                     std::string(_namespace).c_str(), std::string(_name).c_str());
        _data = nullptr;
        return;
    }
    ...
    BNM_LOG_WARN("Class not found in image: %s.%s in %s", std::string(_namespace).c_str(),
                 std::string(_name).c_str(), image.str().c_str());
}
...
std::vector<FieldBase> Class::GetFields(bool includeParent) const {
    ...
    do {
        void *iter = nullptr;
        while (auto field = Internal::api.il2cpp_class_get_fields(curClass, &iter))
            ret.emplace_back((IL2CPP::FieldInfo *) field);
        if (includeParent)
            curClass = Internal::api.il2cpp_class_get_parent(curClass);
        else
            break;
...
std::vector<MethodBase> Class::GetMethods(bool includeParent) const {
    ...
    do {
        void *iter = nullptr;
        while (auto method = Internal::api.il2cpp_class_get_methods(curClass, &iter))
            ret.emplace_back((IL2CPP::MethodInfo *) method);
        if (includeParent)
            curClass = Internal::api.il2cpp_class_get_parent(curClass);
        else
            break;
...
std::vector<PropertyBase> Class::GetProperties(bool includeParent) const {
    ...
    do {
        void *iter = nullptr;
        while (auto prop = Internal::api.il2cpp_class_get_properties(curClass, &iter))
            ret.emplace_back((IL2CPP::PropertyInfo *) prop);
        if (includeParent)
            curClass = Internal::api.il2cpp_class_get_parent(curClass);
        else
            break;
...
std::vector<EventBase> Class::GetEvents(bool includeParent) const {
    ...
    do {
        void *iter = nullptr;
        while (auto eventInfo = Internal::api.il2cpp_class_get_events(curClass, &iter))
            ret.emplace_back((IL2CPP::EventInfo *) eventInfo);
        if (includeParent)
            curClass = Internal::api.il2cpp_class_get_parent(curClass);
        else
            break;
...
MethodBase Class::GetMethod(const std::string_view &name, int parameters) const {
    ...
    auto method = Internal::IterateMethods(*this, [&name, parameters](IL2CPP::MethodInfo *method) {
        return name == (method->name ? method->name : "") &&
    ...
    BNM_LOG_WARN("Method not found: %s.%s(%d)", _data->namespaze ? _data->namespaze : "",
                 _data->name ? _data->name : "", parameters);
    return {};
}
...
MethodBase Class::GetMethod(const std::string_view &name,
    ...
    auto method = Internal::IterateMethods(*this, [&name, &parameters,
                                                   &parameterNames](IL2CPP::MethodInfo *method) {
        if (name != (method->name ? method->name : "") || method->parameters_count != parameters)
            return false;
        for (uint8_t i = 0; i < parameters; ++i) {
            auto paramName = Internal::api.il2cpp_method_get_param_name(method, i);
            if (parameterNames.begin()[i] != (paramName ? paramName : ""))
                return false;
        }
        return true;
    });
...
MethodBase Class::GetMethod(const std::string_view &name,
    ...
    auto method = Internal::IterateMethods(*this, [&name, parameters,
                                                   &parameterTypes](IL2CPP::MethodInfo *method) {
        if (name != (method->name ? method->name : "") || method->parameters_count != parameters)
            return false;
        for (uint8_t i = 0; i < parameters; ++i) {
            ...
            if (!param)
                return false;
            if (Class(param).GetClass() != parameterTypes.begin()[i].ToIl2CppClass())
                return false;
        }
        return true;
    });
...
PropertyBase Class::GetProperty(const std::string_view &name, const CompileTimeClass &type) const {
    ...
    do {
        ...
        while (auto currentProperty = Internal::api.il2cpp_class_get_properties(curClass, &iter)) {
            ...
            if (name == (propInfo->name ? propInfo->name : "") &&
                PropertyBase{propInfo}.GetType() == typeClass)
                return propInfo;
        }
        curClass = Internal::api.il2cpp_class_get_parent(curClass);
    } while (curClass);
...
EventBase Class::GetEvent(const std::string_view &name) const {
    ...
    do {
        ...
        while (auto currentEvent = Internal::api.il2cpp_class_get_events(curClass, &iter)) {
            ...
            if (name == (eventInfo->name ? eventInfo->name : ""))
                return eventInfo;
        }
        curClass = Internal::api.il2cpp_class_get_parent(curClass);
    } while (curClass);
...
void *Class::NewListInstance() {
    return Internal::customListTemplateClass
               ? Internal::api.il2cpp_object_new(Internal::customListTemplateClass)
               : nullptr;
}
    ...
    for (auto &info : _stack) {
        ...
        switch (info->_baseType) {
            ...
            case _BaseType::Inner: {
                ...
                if (!_loadedClass) {
                    BNM_LOG_WARN("Inner class parent not resolved: %s",
                                 innerInfo->_name ? innerInfo->_name : "(null)");
                    break;
                }
                if (!innerInfo->_name)
                    break;
            ...
            case _BaseType::Modifier: {
                ...
                switch (modifierInfo->_modifierType) {
                    case ModifierType::Pointer:
                        _loadedClass = _loadedClass.GetPointer();
                        break;
                    case ModifierType::Reference:
                        _loadedClass = _loadedClass.GetReference();
                        break;
                    case ModifierType::Array:
                        _loadedClass = _loadedClass.GetArray();
                        break;
