...
#pragma GCC diagnostic pop
            ...
            struct Array {
                ...
                inline T *At(IL2CPP::il2cpp_array_size_t index) {
                    return (T *) ((char *) this + sizeof(IL2CPP::Il2CppArray) + sizeof(T) * index);
                }
                inline T &operator[](IL2CPP::il2cpp_array_size_t index) {
                    return *(T *) ((char *) this + sizeof(IL2CPP::Il2CppArray) + sizeof(T) * index);
                }
                ...
                inline std::vector<T> ToVector() const {
                    std::vector<T> ret;
                    for (IL2CPP::il2cpp_array_size_t i = 0; i < max_length; ++i)
                        ret.push_back((*const_cast<Array<T> *>(this))[i]);
                    return ret;
                }
                ...
                static Array<T> *CreateWithItems(Args &&...args) {
                    ...
                    for (IL2CPP::il2cpp_array_size_t i = 0; i < sizeof...(Args); ++i)
                        (*arr)[i] = values[i];
                    return arr;
                }
            ...
            struct DataIterator {
                ...
                inline bool operator==(const DataIterator &other) const {
            ...
            struct List {
                ...
                inline constexpr List()
                    : obj(), _items(nullptr), _size(0), _version(0), _syncRoot(nullptr) {}
                ...
                inline std::vector<T> ToVector() const {
                    ...
                    for (int32_t i = 0; i < _size; ++i)
                        ret.push_back((*const_cast<List<T> *>(this))[i]);
                    return ret;
                }
                ...
                inline bool GrowIfNeeded(int32_t n) {
                    if (!_items)
                        return Resize(n > 4 ? n : 4);
                    if (_size + n > (int32_t) _items->Size())
                        return Resize(_size + n);
                    return true;
                }
                ...
                inline void Shift(int32_t start, int32_t delta) {
                    ...
                    if (delta < 0)
                        start -= delta;
                    if (start < _size)
                        memmove(_items->GetItems() + start + delta, _items->GetItems() + start,
                                (_size - start) * sizeof(T));
                    _size += delta;
                    if (delta < 0)
                        memset(_items->GetItems() + oldSize + delta, 0, -delta * sizeof(T));
                }
                ...
                inline void Clear() {
                    if (_items && _size > 0)
                        memset(_items->GetItems(), 0, _size * sizeof(T));
                    _size = 0;
                    ++_version;
                }

                inline T GetItem(int32_t index) const {
                    if (index < 0 || index >= _size || !_items)
                        return {};
                ...
                inline void Insert(int32_t index, T item) {
                    if (index < 0 || index > _size)
                        return;
                    if (!_items || _size == (int32_t) _items->Size())
                        if (!GrowIfNeeded(1))
                            return;
                    if (index < _size)
                        memmove(_items->GetItems() + index + 1, _items->GetItems() + index,
                ...
                inline int32_t IndexOf(T item) const {
                    if (!_items)
                        return -1;
                    for (int32_t i = 0; i < _size; ++i)
                        if ((*const_cast<List<T> *>(this)->_items)[i] == item)
                            return i;
                    return -1;
                }
                ...
                inline void CopyTo(Array<T> *arr, int32_t arrIndex) const {
                    if (!_items || !arr || arrIndex < 0 || _size <= 0)
                        return;
                    if ((IL2CPP::il2cpp_array_size_t) (arrIndex + _size) > arr->Size())
                        return;
                    memcpy(arr->GetItems() + arrIndex,
                           const_cast<List<T> *>(this)->_items->GetItems(), _size * sizeof(T));
                }

                inline void *GetSyncRoot() {
                    if (!_syncRoot)
                        _syncRoot =
                            (IL2CPP::Il2CppObject *) PRIVATE_MonoListData::CompareExchange4List(
                                _syncRoot);
                    return _syncRoot;
                }
                inline void *GetEnumerator() { return this; }
                inline DataIterator<T> begin() { return DataIterator<T>(GetItems()); }
                inline DataIterator<T> end() {
                    return DataIterator<T>(_items ? GetItems() + _size : nullptr);
                }
            };
            ...
            namespace PRIVATE_MonoListData {
                ...
                constexpr uint32_t FNV1a(const char *str, size_t n, uint32_t hash = 2166136261U) {
                    return n == 0 ? hash
                                  : FNV1a(str + 1, n - 1, (hash ^ (uint8_t) str[0]) * 16777619U);
                }
                ...
                constexpr uint32_t HashedTypeName() {
                    ...
                    constexpr auto name =
                        wrapped.substr(prefix, wrapped.length() - prefix - suffix);
                    return FNV1a(name);
                }

                IL2CPP::Il2CppClass *TryGetMonoListClass(uint32_t typeHash, MethodData *data,
                                                         size_t count);
                void *CompareExchange4List(void *syncRoot);
                ...
                void InitMonoListVTable(List<T> *list) {
                    ...
                    static std::array<MethodData, 16> namesMap = {
                        MethodData{"RemoveAt", *(void **) &RemoveAt},
                        MethodData{"get_Count", *(void **) &GetSize},
                        MethodData{"Clear", *(void **) &Clear},
                        MethodData{"get_Item", *(void **) &get_Item},
                        MethodData{"set_Item", *(void **) &set_Item},
                        MethodData{"IndexOf", *(void **) &IndexOf},
                        MethodData{"Insert", *(void **) &Insert},
                        MethodData{"get_IsReadOnly", *(void **) &get_false},
                        MethodData{"get_IsFixedSize", *(void **) &get_false},
                        MethodData{"get_IsSynchronized", *(void **) &get_false},
                        MethodData{"Add", *(void **) &Add},
                        MethodData{"Contains", *(void **) &Contains},
                        MethodData{"CopyTo", *(void **) &CopyTo},
                        MethodData{"Remove", *(void **) &Remove},
                        MethodData{"GetEnumerator", *(void **) &GetEnumerator},
                        MethodData{"get_SyncRoot", *(void **) &GetSyncRoot}};
                    list->obj.klass = TryGetMonoListClass(HashedTypeName<Type>(), namesMap.data(),
                                                          namesMap.size());
                }
            }  // namespace PRIVATE_MonoListData
